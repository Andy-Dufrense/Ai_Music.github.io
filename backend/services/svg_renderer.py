...
def _title_bar(svg, w: float, notation_type: str, song_info: dict, song_name: str = ""):
    ...
    svg.append(f'<rect x="0" y="0" width="{w}" height="{TITLE_H}" fill="#fafafa" rx="4"/>')
    svg.append(f'<line x1="0" y1="{TITLE_H}" x2="{w}" y2="{TITLE_H}" stroke="#e94560" stroke-width="2"/>')
    svg.append(f'<text x="{PAD_X}" y="{TITLE_H * 0.40}" font-size="16" font-weight="bold" '
               f'fill="#333" font-family="sans-serif">{nt}</text>')
    if song_name:
        svg.append(f'<text x="{PAD_X}" y="{TITLE_H * 0.78}" font-size="11" fill="#999" '
                   f'font-family="sans-serif">{song_name}</text>')
    info_parts = []
    if kd:
        info_parts.append(kd)
    info_parts.append(f"{ts[0]}/{ts[1]}")
    info_parts.append(f"BPM {bpm}")
    info = "  |  ".join(info_parts)
    svg.append(f'<text x="{w - PAD_X}" y="{TITLE_H * 0.52}" text-anchor="end" '
               f'font-size="12" fill="#666" font-family="sans-serif">{info}</text>')


def _staff_lines(svg, x: float, y: float, w: float, n: int = 5):
    for i in range(n):
        ly = y + i * S
        sw = 0.8 if i in (0, n - 1) else 0.4
        svg.append(f'<line x1="{x}" y1="{ly}" x2="{x + w}" y2="{ly}" stroke="#333" stroke-width="{sw}"/>')
...
def _bar_line(svg, x: float, y: float, n: int = 5, final: bool = False, spacing: float = None):
    ...
    bot = y + (n - 1) * sp
    if final:
        svg.append(f'<line x1="{x}" y1="{y}" x2="{x}" y2="{bot}" stroke="#333" stroke-width="1.0"/>')
        svg.append(f'<line x1="{x + 4}" y1="{y}" x2="{x + 4}" y2="{bot}" stroke="#333" stroke-width="2.5"/>')
    else:
        svg.append(f'<line x1="{x}" y1="{y}" x2="{x}" y2="{bot}" stroke="#333" stroke-width="1.2"/>')


def _measure_num(svg, x: float, y: float, num: int):
    svg.append(f'<text x="{x + 2}" y="{y}" font-size="8" fill="#ccc" font-family="sans-serif">{num}</text>')
...
def _note_head(svg, cx: float, cy: float, duration: str):
    ...
    if is_open:
        svg.append(f'<ellipse cx="{cx}" cy="{cy}" rx="{NOTE_R * 0.85}" ry="{NOTE_R * 0.6}" '
                   f'fill="#fff" stroke="#333" stroke-width="1.2" transform="rotate(-15, {cx}, {cy})"/>')
    else:
        svg.append(f'<ellipse cx="{cx}" cy="{cy}" rx="{NOTE_R * 0.85}" ry="{NOTE_R * 0.6}" '
                   f'fill="#333" stroke="#333" stroke-width="0.5" transform="rotate(-15, {cx}, {cy})"/>')
...
def _chord_diagram(svg, dx: float, dy: float, chord_name: str, degree: str, fingering: list):
    ...
    # Name and degree
    svg.append(f'<text x="{dx + box_w / 2}" y="{dy + 12}" text-anchor="middle" font-size="10" '
               f'font-weight="bold" fill="#e94560" font-family="sans-serif">{degree}</text>')
    svg.append(f'<text x="{dx + box_w / 2}" y="{dy + 23}" text-anchor="middle" font-size="14" '
               f'font-weight="bold" fill="#333" font-family="sans-serif">{chord_name}</text>')
...
def render_piano_svg(score_data: dict) -> str:
    ...
    mpr = 2  # Fewer measures per row for readability
    n_rows = max(1, math.ceil(len(measures) / mpr))
    staff_gap = 10 * S  # More space between treble and bass for lyrics
    sw = 940
    row_h = 2 * (4 * S) + staff_gap + 56
    tw = sw + PAD_X * 2 + 60
    th = PAD_Y + TITLE_H + 16 + n_rows * row_h + 24

    svg = [f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {tw} {th}" width="{tw}" style="max-width:100%;">']
    svg.append(f'<rect width="100%" height="100%" fill="#fff" rx="4"/>')
    _title_bar(svg, tw, "piano", song_info, score_data.get("_songName", ""))

    key_name = song_info.get("key", "C")
    key_mode = song_info.get("keyMode", "major")

    for row in range(n_rows):
        rm = measures[row * mpr:(row + 1) * mpr]
        nm = max(1, len(rm))
        mw = sw / nm

        by = PAD_Y + TITLE_H + 18 + row * row_h
        ty = by
        bass_y = ty + 4 * S + staff_gap

        # Brace (first row only)
        if row == 0:
            svg.append(f'<text x="{PAD_X - 14}" y="{ty + 4 * S + 2}" '
                       f'font-size="{8 * S}" font-family="serif" fill="#777">𝄻</text>')

        # Clefs on every system
        svg.append(f'<text x="{PAD_X + 4}" y="{ty + 3.4 * S}" font-size="{S * 4}" '
                   f'font-family="serif" fill="#333">𝄞</text>')
        svg.append(f'<text x="{PAD_X + 4}" y="{bass_y + 1.5 * S}" font-size="{S * 3.5}" '
                   f'font-family="serif" fill="#333">𝄢</text>')

        # Key signatures (after clefs)
        ...
        # Extra left margin to accommodate clef + key sig + time sig
        ...
        for i, measure in enumerate(rm):
            ...
            _DYN_MAP = {"前奏":"p","主歌":"mp","副歌":"f","桥段":"mf","间奏":"mp","尾奏":"p"}
            for sec in sections:
                if sec.get("start_measure") == mi:
                    label = sec.get("label", "")
                    svg.append(f'<text x="{mx + mw2 / 2}" y="{ty - 2.5 * S}" text-anchor="middle" '
                               f'font-size="11" font-weight="bold" fill="#e94560" font-family="sans-serif">[{label}]</text>')
                    dyn_mark = _DYN_MAP.get(label, "mf")
                    svg.append(f'<text x="{mx + 4}" y="{ty - 0.5 * S}" font-size="10" font-weight="bold" '
                               f'fill="#555" font-family="serif" font-style="italic">{dyn_mark}</text>')
            ...
            if tn:
                # Add chord degree to first note without mutating original
                tn_render = [dict(n) for n in tn]
                if cd:
                    tn_render[0] = {**tn_render[0], "chordDeg": cd}
                _draw_measure_notes(svg, tn_render, ty, mx, mw2, song_info.get("timeSignature", [4, 4])[0], "treble",
                                    show_lyrics=False, key_name=key_name, key_mode=key_mode)

            # Bass notes
            bn = measure.get("bassNotes", [])
            if bn:
                _draw_measure_notes(svg, bn, bass_y, mx, mw2, song_info.get("timeSignature", [4, 4])[0], "bass",
                                    show_lyrics=False, key_name=key_name, key_mode=key_mode)

            # Bar line
            bx = mx + mw2
            is_last = (row == n_rows - 1 and i == nm - 1)
...
def render_guitar_svg(score_data: dict) -> str:
    """Render guitar TAB with lyrics, jianpu, chord accompaniment."""
    measures = score_data.get("measures", [])
    song_info = score_data.get("songInfo", {})
    sections = score_data.get("sections", [])
    mpr = 2
    ...
    n_chords = len(chord_set)
    chord_rows = math.ceil(n_chords / 5) if n_chords > 0 else 0
    chord_h = chord_rows * 82 + 12 if chord_rows > 0 else 0
    # Row height: lyrics+jianpu area (~40) + chord label (20) + TAB lines (~58) + gap
    row_h = 210
    tw = sw + PAD_X * 2 + 20
    th = PAD_Y + TITLE_H + chord_h + 16 + n_rows * row_h + 28

    svg = [f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {tw} {th}" width="{tw}" style="max-width:100%;">']
    svg.append(f'<rect width="100%" height="100%" fill="#fff" rx="4"/>')
    _title_bar(svg, tw, "guitar", song_info, score_data.get("_songName", ""))

    content_top = PAD_Y + TITLE_H + 10

    # Chord diagrams at top
    if chord_set:
    ...
    for row in range(n_rows):
        ...
        for i, measure in enumerate(rm):
            ...
            for sec in sections:
                if sec.get("start_measure") == mi:
                    label = sec.get("label", "")
                    svg.append(f'<text x="{mx + mw2 / 2}" y="{tab_y + 10}" text-anchor="middle" '
                               f'font-size="11" font-weight="bold" fill="#e94560" font-family="sans-serif">[{label}]</text>')
                    dyn_mark = _DYN_MAP.get(label, "mf")
                    svg.append(f'<text x="{mx + 168}" y="{tab_y + 10}" font-size="10" font-weight="bold" '
                               f'fill="#555" font-family="serif" font-style="italic">{dyn_mark}</text>')

            # ---- Chord name at measure start ----
            chord_y = tab_y + 24
            if chord:
                svg.append(f'<text x="{mx + 4}" y="{chord_y}" text-anchor="start" '
                           f'font-size="13" font-weight="bold" fill="#222" font-family="sans-serif">{chord.get("chord", "")}</text>')
            ...
            # String labels (first row, first measure)
            if row == 0 and i == 0:
            ...
            # ---- TAB pattern (picking or strumming) ----
            tab_bottom = line_top + 5 * 9  # bottom of last string
            if tab_notes:
                total_beats = song_info.get("timeSignature", [4, 4])[0]
                draw_w = mw2 - 10

                if play_style == "picking" or play_style == "hybrid":
                    # Mixed: each note renders individually based on isPicking flag
                    # Separate picking and strumming notes for beaming
                    ...
                    for pni, (ni, note) in enumerate(picking_notes):
                        if current_group:
                            ...
                            # Only beam adjacent picking notes (no strumming in between)
                    ...
                    for ni, note in enumerate(tab_notes):
                        ...
                        if note.get("isPicking"):
                            ...
                            str_y = line_top + (str_num - 1) * 9  # str 1=top, 6=bottom
                    ...
                    # Draw beams for grouped notes above TAB
                    for bg in beam_groups:
                        if len(bg) < 2:
                            continue
                        stem_top = line_top - 8
                    ...
                    # Strumming: arrows only — no stems, no flags
                    for ni, note in enumerate(tab_notes):
                        ...
                        arr_top = line_top - 4   # Gap above TAB
                        arr_bot = tab_bottom + 3  # Gap below TAB
            ...
            if lyric_notes:
                ...
                for ln in lyric_notes:
                    ...
                    # Lyric text below jianpu
                    if lyric_text:
                        svg.append(f'<text x="{nx}" y="{lyric_y}" text-anchor="middle" '
                                   f'font-size="13" fill="#333" font-family="sans-serif">{lyric_text}</text>')
...
def render_bass_svg(score_data: dict) -> str:
    ...
    row_h = 105
    tw = sw + PAD_X * 2 + 20
    th = PAD_Y + TITLE_H + 20 + n_rows * row_h + 24

    svg = [f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {tw} {th}" width="{tw}" style="max-width:100%;">']
    svg.append(f'<rect width="100%" height="100%" fill="#fff" rx="4"/>')
    _title_bar(svg, tw, "bass", song_info, score_data.get("_songName", ""))
    ...
    for row in range(n_rows):
        ...
        for i, measure in enumerate(rm):
            ...
            for sec in sections:
                if sec.get("start_measure") == mi:
                    svg.append(f'<text x="{mx + mw2 / 2}" y="{tab_y + 8}" text-anchor="middle" '
                               f'font-size="9" font-weight="bold" fill="#e94560" font-family="sans-serif">[{sec.get("label", "")}]</text>')
            ...
            if tnotes:
                ...
                for ni, note in enumerate(tnotes):
                    ...
                    for pos in positions:
                        ...
                        svg.append(f'<text x="{nx}" y="{ny}" text-anchor="middle" font-size="11" '
                                   f'font-weight="bold" fill="#333" font-family="sans-serif">{fret}</text>')
                        prev_x = nx
                        prev_str = string

                    # Jianpu degree above TAB
...
def render_drums_svg(score_data: dict) -> str:
    ...
    svg = [f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {tw} {th}" width="{tw}" style="max-width:100%;">']
    svg.append(f'<rect width="100%" height="100%" fill="#fff" rx="4"/>')
    _title_bar(svg, tw, "drums", song_info, score_data.get("_songName", ""))
        ...
        for i, measure in enumerate(rm):
            ...
            if row == 0 and i == 0:
                ...
                svg.append(f'<text x="{mx + 50}" y="{sy + 3 * S}" font-size="14" '
                           f'font-family="serif" font-weight="bold" fill="#333">{ts[1]}</text>')
                svg.append(f'<text x="{mx}" y="{sy + 6 * S}" font-size="8" fill="#aaa">'
                           f'● Kick  ● Snare  × HH</text>')
